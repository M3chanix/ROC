...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QAction, QFileDialog, QComboBox, QPushButton, \
    QGridLayout, QLabel, QTableWidget, QTableWidgetItem, QLineEdit, QInputDialog, QDialog, QCheckBox
os.chdir("C://py1")


def parse_file(fname):
    file = pd.read_excel(fname, skiprows=19)
    unique_ids = file["Well Note"].unique()
    ...
    Cq = file["Cq"].to_numpy()
    Cq.resize(len(unique_ids), int(len(file)/len(unique_ids)))
    labels = file["Sample"].unique()
    patient_table = pd.DataFrame(Cq, columns=labels)
    patient_table.insert(0, "Sample ID", unique_ids)
    tissue = np.array([])
    for i in range(int(len(unique_ids))):
        tissue = np.append(tissue, file["Target"].loc[12*i])
...
class NewDataWindow(QWidget):
    ...
    def prepareAnalyzis(self):
        ...
        # todo: перед сохранением в базу дополнить до всех именований miRNA
        # todo: просто написать запрос с использованием insert into table_name column_list values()
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setGeometry(300, 200, 900, 450)
        new_data = NewDataWindow()
        self.setCentralWidget(new_data)
        self.show()
...
class ResultsWindow(QWidget):
    ...
    def normalization(self, raw_data):
        # todo: хз, че делать с NAN
        # raw_data = raw_data.dropna(axis=0)

        column_names = list(raw_data)
        norm_data = pd.DataFrame(raw_data, columns=raw_data.columns[:2])
        for divident in column_names[2:]:
            for divider in column_names[2:]:
                if divident != divider:
